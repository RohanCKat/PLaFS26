...
let is_subclass: string -> string -> class_env -> exp_val ea_result = 
  fun c p c_env ->
  let rec helper child parent class_env =
    fun env ->
    (match List.assoc_opt child class_env with
      | None -> error ("Is Subclass: class "^child^" not found")
      | Some (super,_fields,_methods) -> if super=parent then return (BoolVal (true)) else (if super="object" then return (BoolVal (false)) else helper (super) (child) (class_env) (env)))
  in if c=p then return (BoolVal (true)) else helper c p c_env env
...
and   
  ...
  | Super(m_name,es) ->
    ...
    (match lookup_method c_name (name_mangle m_name args) !g_class_env with
     ...
     | Some m -> apply_method (name_mangle m_name args) self args m)
  | IsInstanceOf(e, id) ->
    eval_expr e >>= fun self -> obj_of_objectVal self >>= fun (name,_) -> is_subclass name id !g_class_env
  (* List operations* *)
  | List(es) ->
    eval_exprs es >>= fun args ->
    return @@ ListVal args
